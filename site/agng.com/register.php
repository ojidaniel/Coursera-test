...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["birthday"]))){
        $birthday_err = "Please enter a Date.";
    } else{
        // Prepare a select statement
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            $param_birthday = date('Y-m-d', strtotime(str_replace("/","-", $_POST['birthday'])));
    ...
    if(empty($first_name_err) && empty($last_name_err) && empty($gender_err) && empty($birthday_err) && empty($email_err) && empty($mobile_err) && empty($password_err) && empty($confirm_password_err)){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters            
            mysqli_stmt_bind_param($stmt, "sssssss", $param_first_name, $param_last_name, $param_gender, $param_birthday, $param_email, $param_mobile, $param_password);
        ...
        <div class="wrapper wrapper--w960">
            <div class="card card-2">
                ...
                <div class="card-body">
                    ...
                    <form id="js-form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        ...
                        <div class="row row-space">
                            ...
                            <div class="col-2">
                                <div class="input-group <?php echo (!empty($birthday_err)) ? 'has-error' : ''; ?>">
                                    <!--  (For Datepicker) class="input--style-2 js-datepicker"  -->
                                    <input id="datepicker" class="input--style-2 js-datepicker" type="text" placeholder="Birthday" name="birthday" value="<?php echo $birthday; ?>" autocomplete="off" readonly onfocus="this.placeholder='??/??/????'" onblur="this.placeholder='Birthday'" size=10 maxlength=10 onkeyup="this.value=this.value.replace(/^(\d\d)(\d)$/g,'$1/$2').replace(/^(\d\d\/\d\d)(\d+)$/g,'$1/$2').replace(/[^\d\/]/g,'')">
                    ...
                    <p style="margin-top: 50px; text-align: center;"><a style=" text-decoration: none; color: #666666;" href="index.php">Home</a>&nbsp;|&nbsp;<a style=" text-decoration: none; color: #666666;" href="login.php">Sign In</a></p>
                </div>
            </div>
        </div>
    </div>
